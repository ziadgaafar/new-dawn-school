      ...
      <Row className="welcome-div justify-content-center ml-1 mt-5 w-100">
        <Col xs={10} md={7}>
          <div className="float-left">
            <h1 className="student-header fw-bold fs-2 " style={{ color: "#0040A1" }}>
              Welcome Back, {user.firstName} !
            </h1>
            <p className="student-p">
              You Have Learned<span className="fw-bold">70%</span> Of Your Goal This Week! <br></br> Keep It Up And Improve Your Progress!
            </p>
          </div>
      ...
      <Row className="justify-content-center ml-1 mt-5 w-100">
        <Col lg={6} xs={12} className="student-infos mt-5 bg-white m-2">
          ...
          <Row className="student-infos p-4">
            ...
            <Col xs={12} md={6} className="float-right">
              <button className="week-but float-right">Last Week <i className="arrow down"></i></button>
            </Col>
          </Row>
          <Row className="w-100">
            <Chart
        ...
        <Col lg={5} xs={12} className=" mt-5 m-2">
          <div className="student-infos bg-white p-4">
            <Row>
              <h3 style={{ color: "#0040A1" }} className=" " > Time Spent </h3>
            </Row>
              ...
              {subjectsDegree.map((item, index) => {
                return (
                  <ul className="row" key={index}>
                    <Col ><div className="square" style={{ 'backgroundColor': item.color }}></div> {item.subject}</Col>
        ...
        <Row className="mt-5">
          <Row className="courses-attend p-4">
            ...
            {coursesAttend.map((item, index) => {
              return (
                <Col lg={5} key={index} className="courses-attend m-2 rounded p-3  ">
                  <Row >
                    <Col xs={2} lg={1}><div className="circle" style={{ 'backgroundColor': item.background }}></div></Col>
                    <Col xs={7}><h3 className="ml-3">{item.subject}</h3></Col>
                    <Col xs={12} md={1}> <p className="text-muted float-right">{item.time}</p></Col>
                  </Row>
